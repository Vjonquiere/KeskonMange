...
beforeAll(async () => {
  ...
  userId = (
    await conn.query("SELECT userId FROM authentication WHERE token = ?;", [
      ApiKeyHash(ID["x-api-key"]),
    ])
  )[0].userId;
  // Sample data for self-adapt data
  await conn.query(
    `INSERT INTO calendar VALUES ('${dateFromToday(0)}', 0, 0, 'path/to/image', ?);`,
    [userId],
  );
  await conn.query(
    `INSERT INTO calendar VALUES ('${dateFromToday(1)}', 4, 1, 'path/to/image', ?);`,
    [userId],
  );
  await conn.query(
    `INSERT INTO calendar VALUES ('${dateFromToday(-1)}', 1, 1, 'path/to/image', ?);`,
    [userId],
  );
  await conn.query(
    `INSERT INTO calendar VALUES ('${dateFromToday(3)}', 5, 0, 'path/to/image', ?);`,
    [userId],
  );
  // Sample data for September 2024
  await conn.query(
    `INSERT INTO calendar VALUES ('2024-09-01', 0, 0, 'path/to/image', ?);`,
    [userId],
  );
  await conn.query(
    `INSERT INTO calendar VALUES ('2024-09-02', 4, 1, 'path/to/image', ?);`,
    [userId],
  );
  await conn.query(
    `INSERT INTO calendar VALUES ('2024-09-03', 1, 1, 'path/to/image', ?);`,
    [userId],
  );
  await conn.query(
    `INSERT INTO calendar VALUES ('2024-09-04', 5, 0, 'path/to/image', ?);`,
    [userId],
  );
  // Sample data for other months
  await conn.query(
    `INSERT INTO calendar VALUES ('2023-01-03', 1, 4, 'path/to/image', ?);`,
    [userId],
  );
  await conn.query(
    `INSERT INTO calendar VALUES ('2023-12-20', 6, 1, 'path/to/image', ?);`,
    [userId],
  );
  await conn.query(
    `INSERT INTO calendar VALUES ('2022-09-03', 1, 4, 'path/to/image', ?);`,
    [userId],
  );
});
...
describe("GET calendar/today", () => {
  it("simple call", async () => {
    ...
    expect(res.text).toBe(
      `{\"recipe\":[{\"date\":\"${dateFromToday(0)}\",\"recipeId\":0,\"done\":0,\"result_img\":\"path/to/image\"}]}`,
    ); // Given entry formatted to JSON
  });
  it("call with no recipe defined", async () => {
    await conn.query(
      `DELETE FROM calendar WHERE date = '${dateFromToday(0)}';`,
    );
    const res = await request(app).get("/calendar/today").set(ID).send();
...
describe("GET calendar/coming", () => {
  it("call on known value (day+1)", async () => {
    const res = await request(app)
      .get("/calendar/coming?days=1")
      .set(ID)
      .send();
    expect(res.status).toBe(200);
    expect(res.text).toBe(
      `{\"recipe\":[{\"date\":\"${dateFromToday(1)}\",\"recipeId\":4,\"done\":1,\"result_img\":\"path/to/image\"}]}`,
    ); // Given entry formatted to JSON
  });
  it("call on known value (negative day number) (day-1)", async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.text).toBe(
      `{\"recipe\":[{\"date\":\"${dateFromToday(-1)}\",\"recipeId\":1,\"done\":1,\"result_img\":\"path/to/image\"}]}`,
    ); // Given entry formatted to JSON
  });
  it("call on known value with no recipe (day+10)", async () => {
    const res = await request(app)
      .get("/calendar/coming?days=10")
      .set(ID)
      .send();
    expect(res.status).toBe(204);
  });
...
describe("GET calendar/next", () => {
  ...
  it("call with no next recipe defined", async () => {
    await conn.query(
      `DELETE FROM calendar WHERE date = '${dateFromToday(1)}';`,
    ); // removing known entries
    await conn.query(
      `DELETE FROM calendar WHERE date = '${dateFromToday(3)}';`,
    );
    const res = await request(app).get("/calendar/next?count=2").set(ID).send();
    expect(res.status).toBe(204);
    await conn.query(
...
describe("GET calendar/completeMonth", () => {
  it("simple call", async () => {
    const res = await request(app)
      .get("/calendar/completeMonth?previous=0")
      .set(ID)
      .send();
  ...
  it("call on wrong argument (previous)", async () => {
    const res = await request(app)
      .get("/calendar/completeMonth?previous=Nan")
      .set(ID)
      .send();
    expect(res.status).toBe(405);
  });
  it("call on empty month", async () => {
    const res = await request(app)
      .get("/calendar/completeMonth?previous=-2")
      .set(ID)
      .send(); // -2 to get 2 months in the future
