...
async function getRecipeId(name) {
  ...
  const [res] = await conn.query(
    "SELECT id FROM recipes WHERE owner = ? AND title = ?;",
    [userId.id, name],
  );
  return res.id;
}
...
describe("POST recipe/add", () => {
    ...
    const res = await request(app)
      ...
      .send({
        ingredients: [
          ...
          { name: "Viande hachee", qte: 100, unit: "g", type: "meat" },
          { name: "Sauce tomate", qte: 100, unit: "g", type: "grocerie" },
          { name: "Fromage", qte: 100, unit: "g", type: "grocerie" },
        ],
        preparation_time: 10,
        rest_time: 0,
        cook_time: 10,
  ...
  it("call on invalid argument (ingredient with unknown unit)", async () => {
    const res = await request(app)
      ...
      .send({
        ingredients: [
          { name: "Pates", qte: 200, unit: "l", type: "grocerie" },
          { name: "Viande hachee", qte: 100, unit: "g", type: "meat" },
          { name: "Sauce tomate", qte: 100, unit: "g", type: "grocerie" },
        ],
        preparation_time: 10,
...
describe("POST recipe/image", () => {
  it("call on valid arguments", async () => {
    ...
    const serverFilePathBase = path.join(
      __dirname,
      `../public/images/recipe/${recipeId}`,
    );
    const response = await request(app)
      .post(`/recipe/image?recipeId=${recipeId}`)
      .set(ID)
      .attach("image", resPath);
...
describe("GET recipe/image", () => {
  it("call on valid arguments", async () => {
    ...
    await sleep(500); // Wait for the files to be created
    const res = await request(app)
      .get(`/recipe/image?recipeId=${recipeId}&format=16_9`)
      .set(ID);
    expect(res.status).toBe(200);
  });
  it("call on missing argument (recipeId)", async () => {
  ...
  it("call on missing argument (format)", async () => {
    const res = await request(app)
      .get(`/recipe/image?recipeId=${1000}`)
      .set(ID);
    expect(res.status).toBe(405);
  });
  it("call on unkown recipeId", async () => {
    const res = await request(app)
      .get(`/recipe/image?recipeId=${1000}&format=16_9`)
      .set(ID);
...
describe("POST recipe/steps", () => {
  it("call on valid arguments", async () => {
    ...
    const response = await request(app)
      .post(`/recipe/steps?recipeId=${recipeId}`)
      .set(ID)
      .send({
        steps: [
  ...
  it("call on missing argument (steps)", async () => {
    const response = await request(app)
      .post(`/recipe/steps?recipeId=${1000}`)
      .set(ID)
      .send();
    expect(response.status).toBe(405);
  });
...
describe("GET recipe/steps", () => {
  it("call on valid arguments", async () => {
    ...
    await sleep(500); // Wait for the files to be created
    const res = await request(app)
      .get(`/recipe/steps?recipeId=${recipeId}`)
      .set(ID)
      .send();
    expect(res.status).toBe(200);
  ...
  it("call on unknown recipe", async () => {
    const res = await request(app)
      .get(`/recipe/steps?recipeId=${1000}`)
      .set(ID)
      .send();
    expect(res.status).toBe(204);
  });
