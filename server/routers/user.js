...
router.get("/availableUsername", async (req, res) => {
  ...
  const usernameUnique = await conn.query(
  ...
  if (Array.from(usernameUnique)[0]["COUNT(id)"] > 0) {
    res
      .status(405)
      .send("This username is taken by another user, find a new one!");
    return;
  }
  res.sendStatus(200);
});
...
router.get("/availableEmail", async (req, res) => {
  ...
  const emailUnique = await conn.query(
...
router.post("/create", async (req, res) => {
  if (req.query.email === undefined || req.query.username === undefined) {
    res
      .status(405)
      .send("An email and a username are required to create an account");
    return;
  }
  ...
  if (
    ...
    !usernameRegex.test(req.query.username)
  ) {
  ...
  await conn.query("INSERT INTO verify VALUES (?, ?);", [
    req.query.email,
    verificationCode,
  ]);
  await conn.query("INSERT INTO users VALUES (NULL, ?, ?, NULL);", [
    req.query.email,
    req.query.username,
  ]);
...
router.post("/verify", async (req, res) => {
  ...
  const user = await conn.query(
    "SELECT email FROM verify WHERE email = ? AND code = ?;",
  ...
  if (arrayUser.length > 0 && arrayUser[0]["email"] == req.query.email) {
    ...
    await conn.query("UPDATE users SET verified = ? WHERE email = ?;", [
      formattedDate,
      req.query.email,
    ]);
  ...
  if (req.body.hasOwnProperty("allergens")) {
    ...
    for (let i = 0; i < allergens.length; i++) {
      if (!isNaN(Number(allergens[i])) && allergens[i] < ALLERGEN_NUMBER) {
        conn.query("INSERT INTO allergens VALUES (?, ?);", [
          req.user.userId,
          allergens[i],
        ]);
      }
    ...
    const allergens = await conn.query(
      "SELECT allergenId AS allergen FROM allergens JOIN users on allergens.userId = users.id WHERE users.email = ?;",
      [req.query.email],
    );
