...
async function hasAccess(recipeId, userId) {
  const isPublic = await conn.query(
    "SELECT id FROM recipes WHERE id = ? AND visibility = 0;",
    [recipeId],
  );
  const isOwner = await conn.query(
    "SELECT id FROM recipes WHERE id = ? AND owner = ?;",
    [recipeId, userId],
  );
...
router.get("/complete", async (req, res) => {
  const recipeData = await conn.query(
    "SELECT * FROM recipes JOIN durations ON recipes.id = durations.recipeId WHERE recipes.id=?;",
    [req.body.id],
  );
  const ingredienList = await conn.query(
    "SELECT ingredients.name FROM recipes_ingredients_link JOIN ingredients ON ingredients.id = recipes_ingredients_link.ingredientId WHERE recipes_ingredients_link.recipeId=?;",
    [req.body.id],
  );
  res.json(
    JSON.stringify({ recipe: recipeData[0], ingredients: ingredienList }),
  );
});
...
router.get("/last", async (req, res) => {
  const result = await conn.query(
    "SELECT * FROM recipes WHERE visibility = 0 ORDER BY id DESC;",
  );
  res.json(result[0]);
});
...
router.get("/image", needAuth, async (req, res) => {
  ...
  if (!(await hasAccess(req.query.recipeId, req.user.userId)))
...
router.post(
  ...
  async (req, res) => {
    ...
    sharp(req.file.buffer)
      .resize(1280, 720)
      .toFile(imagePath + "16_9.png");
    sharp(req.file.buffer)
      .resize(720, 720)
      .toFile(imagePath + "1_1.png");
    sharp(req.file.buffer)
      .resize(1280, 960)
      .toFile(imagePath + "4_3.png");
...
router.post("/add", needAuth, async (req, res) => {
  ...
  try {
    conn.query(
      "INSERT INTO recipes VALUES (null, ?, ?, ?, ?, ?, null, null, null, null, null, ?, ?, ?, 0);",
      [
        req.body.title,
        req.body.type,
        Number(req.body.difficulty),
        Number(req.body.cost),
        Number(req.body.portions),
        req.body.salty,
        req.body.sweet,
        req.user.userId,
      ],
...
router.get("/:id", needAuth, async (req, res) => {
  //TODO: change params to query + code 204 if no recipe found
  if (req.params.id === undefined || isNaN(Number(req.params.id))) {
    res.status(405).send("undifined recipe_id");
    return;
  }
  try {
    if (!(await hasAccess(req.params.id, req.user.userId)))
      return res.sendStatus(204);
    const result = await conn.query("SELECT * FROM recipes WHERE id = ?;", [
      req.params.id,
    ]);
    res.json(result[0]);
  } catch (error) {
