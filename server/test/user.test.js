...
describe("POST user/create", () => {
  it("simple call", async () => {
    const res = await request(app).post(
      `/user/create?email=${process.env.POP3_ADDRESS}&username=test`,
    );
  ...
  it("call with missing parameter (username)", async () => {
    const res = await request(app).post(
      `/user/create?email=${process.env.POP3_ADDRESS}`,
    );
    expect(res.status).toBe(405);
  });
  it("call on wrong email", async () => {
    const res = await request(app).post(
      "/user/create?email=test%40test&username=test",
    );
    expect(res.status).toBe(405);
  });
  it("call on wrong username (invalid character)", async () => {
    const res = await request(app).post(
      "/user/create?email=test%40test.com&username=test@1",
    );
    expect(res.status).toBe(405);
  });
  it("call on wrong username (invalid length)", async () => {
    const res = await request(app).post(
      "/user/create?email=test%40test.com&username=test1234567891011",
    );
    expect(res.status).toBe(405);
  });
  it("email already used", async () => {
    const res = await request(app).post(
      `/user/create?email=${process.env.POP3_ADDRESS}&username=test2`,
    );
    expect(res.status).toBe(405);
  ...
  it("username already used", async () => {
    const res = await request(app).post(
      `/user/create?email=${process.env.POP3_ADDRESS}m&username=test`,
    );
    expect(res.status).toBe(405);
    expect(res.text).toBe(
      "This username is taken by another user, find a new one!",
    );
  });
});
...
describe("POST user/verify", () => {
  it("simple call", async () => {
    const verificationCode = Array.from(
      await conn.query("SELECT code FROM verify WHERE email = ?;", [
        process.env.POP3_ADDRESS,
      ]),
    )[0]["code"]; // User previously created
    console.log(verificationCode);
    const res = await request(app).post(
      `/user/verify?email=${process.env.POP3_ADDRESS}&code=${verificationCode}`,
    );
    expect(res.status).toBe(200);
  });
  it("call on unknown values", async () => {
    const res = await request(app).post(
      `/user/verify?email=unknown%40test.com&code=0000`,
    );
  ...
  it("call on missing parameter (code)", async () => {
    const res = await request(app).post(
      `/user/verify?email=unknown%40test.com`,
    );
    expect(res.status).toBe(405);
  });
});
...
describe("GET user/availableUsername", () => {
  it("simple call", async () => {
    const res = await request(app).get(
      `/user/availableUsername?username=IsAvailable`,
    );
    expect(res.status).toBe(200);
  });
