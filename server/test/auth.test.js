...
beforeAll(async () => {
  ...
  await conn.query(
    "INSERT INTO users VALUES (NULL, ?, 'test', '2025-01-06');",
...
describe("POST auth/signin", () => {
  it("simple call", async () => {
    const res = await request(app)
      .post(`/auth/signin?email=${process.env.POP3_ADDRESS}&lang=debug`)
      .send();
    ...
    const res2 = await request(app)
      .post(
        `/auth/signin?email=${process.env.POP3_ADDRESS}&lang=debug&code=${parsedEmail.subject}`,
      )
      .send();
  ...
  it("call on invalid email (no account created with it)", async () => {
    const res = await request(app)
      .post(`/auth/signin?email=test120%40test.com&lang=debug`)
      .send();
  ...
  it("call on invalid data (no matching between email and code)", async () => {
    const res = await request(app)
      .post(`/auth/signin?email=test120%40test.com&lang=debug&code=2131`)
      .send();
    expect(res.status).toBe(404);
  });
});
...
describe("GET auth/test", () => {
  it("simple call", async () => {
    const res = await request(app)
      .post(
        `/auth/test?email=${process.env.POP3_ADDRESS}&api_key=${connexionToken}`,
      )
      .send();
  ...
  it("call on missing argument (email)", async () => {
    const res = await request(app)
      .post(`/auth/test?&api_key=${connexionToken}`)
      .send();
    expect(res.status).toBe(405);
  });
  it("call on missing argument (api_key)", async () => {
    const res = await request(app)
      .post(`/auth/signin?api_key=${connexionToken}`)
      .send();
    expect(res.status).toBe(405);
  });
...
describe("GET auth/logout", () => {
  it("simple call", async () => {
    const res = await request(app)
      .post(
        `/auth/logout?email=${process.env.POP3_ADDRESS}&api_key=${connexionToken}`,
      )
      .send();
    expect(res.status).toBe(200);
  });
  it("call on missing argument (email)", async () => {
    const res = await request(app)
      .post(`/auth/logout?&api_key=${connexionToken}`)
      .send();
    expect(res.status).toBe(405);
  });
  it("call on missing argument (api_key)", async () => {
