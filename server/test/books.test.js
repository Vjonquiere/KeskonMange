...
describe("POST books/create", () => {
  it("simple call", async () => {
    const res = await request(app)
      .post("/books/create?name=test")
      .set(ID)
      .send();
    expect(res.status).toBe(200);
    const check = await request(app)
      .get("/books/id?bookName=test")
      .set(ID)
      .send(); // check if we can get id of the book
    expect(check.status).toBe(200);
    const check2 = await request(app)
      .get(`/books/general_information?bookId=${JSON.parse(check.text)["id"]}`)
      .set(ID)
      .send(); // check link between bookId and book name
  ...
  it("call on wrong argument (invalid len)", async () => {
    const res = await request(app)
      .post("/books/create?name=theNameOfThisBookIsSoLoooooooooooong")
      .set(ID)
      .send();
    expect(res.status).toBe(400);
  });
...
describe("DELETE books/delete", () => {
  it("simple call", async () => {
    const res = await request(app)
      .post("/books/create?name=testDelete")
      .set(ID)
      .send();
    expect(res.status).toBe(200);
    const check = await request(app)
      .get("/books/id?bookName=testDelete")
      .set(ID)
      .send(); // check if we can get id of the book
    expect(check.status).toBe(200);
    const check2 = await request(app)
      .delete(`/books/delete?bookId=${JSON.parse(check.text)["id"]}`)
      .set(ID)
      .send(); // check if book can be deleted
    expect(check2.status).toBe(200);
  });
  it("call on missing argument", async () => {
  ...
  it("call on not indexed bookId", async () => {
    const check2 = await request(app)
      .delete(`/books/delete?bookId=test3`)
      .set(ID)
      .send();
    expect(check2.status).toBe(200);
  });
...
describe("POST books/share", () => {
  it("simple call", async () => {
    const res = await request(app)
      .post("/books/create?name=test2")
      .set(ID)
      .send();
    expect(res.status).toBe(200);
    const check = await request(app)
      .get("/books/id?bookName=test2")
      .set(ID)
      .send(); // check if we can get id of the book
    expect(check.status).toBe(200);
    const share = await request(app)
      .post(`/books/share?bookId=${JSON.parse(check.text)["id"]}&userId=23`)
      .set(ID)
      .send();
  ...
  it("call on missing argument (userId)", async () => {
    const res = await request(app)
      .post("/books/create?name=test3")
      .set(ID)
      .send();
    expect(res.status).toBe(200);
    const check = await request(app)
      .get("/books/id?bookName=test3")
      .set(ID)
      .send(); // check if we can get id of the book
    expect(check.status).toBe(200);
    const share = await request(app)
      .post(`/books/share?bookId=${JSON.parse(check.text)["id"]}`)
      .set(ID)
      .send();
    expect(share.status).toBe(400);
  });
  it("call on wrong bookId", async () => {
    const res = await request(app)
      .post("/books/create?name=test4")
      .set(ID)
      .send();
    expect(res.status).toBe(200);
    const check = await request(app)
      .get("/books/id?bookName=test4")
      .set(ID)
      .send(); // check if we can get id of the book
    expect(check.status).toBe(200);
    const share = await request(app)
      .post(
        `/books/share?bookId=${JSON.parse(check.text)["id"] + 1000}&userId=23`,
      )
      .set(ID)
      .send(); // DB is reset before testing -> number of recipe books <1000
...
describe("POST books/recipe/add", () => {
  it("simple call", async () => {
    const res = await request(app)
      .post("/books/create?name=recipeAddTest")
      .set(ID)
      .send();
    expect(res.status).toBe(200);
    const check = await request(app)
      .get("/books/id?bookName=recipeAddTest")
      .set(ID)
      .send();
    expect(check.status).toBe(200);
    const add = await request(app)
      .post(
        `/books/recipe/add?bookId=${JSON.parse(check.text)["id"]}&recipeId=21`,
      )
      .set(ID)
      .send();
  ...
  it("add to unknown book", async () => {
    ...
    const add = await request(app)
      .post(
        `/books/recipe/add?bookId=${JSON.parse(check.text)["id"] + 1000}&recipeId=21`,
      )
      .set(ID)
      .send();
  ...
  it("call on missing argument (bookId)", async () => {
    const add = await request(app)
      .post(`/books/recipe/add?recipeId=21`)
      .set(ID)
      .send();
  ...
  it("call on missing argument (recipeId)", async () => {
    const add = await request(app)
      .post(`/books/recipe/add?bookId=1`)
      .set(ID)
      .send();
    expect(add.status).toBe(400);
    expect(add.text).toBe("You need to specify a bookId and a recipeId");
...
describe("GET books/recipes", () => {
  it("simple call", async () => {
    const res = await request(app)
      .post("/books/create?name=recipesTest")
      .set(ID)
      .send();
    const check = await request(app)
      .get("/books/id?bookName=recipesTest")
      .set(ID)
      .send();
    expect(check.status).toBe(200);
    expect(res.status).toBe(200);
...
describe("GET books/general_information", () => {
  it("simple call", async () => {
    const res = await request(app)
      .post("/books/create?name=recipesGeneralInfos")
      .set(ID)
      .send();
    const check = await request(app)
      .get("/books/id?bookName=recipesGeneralInfos")
      .set(ID)
      .send();
    expect(check.status).toBe(200);
    expect(res.status).toBe(200);
    ...
    const rawInfos = await request(app)
      .get(`/books/general_information?bookId=${id}`)
      .set(ID);
  ...
  it("call on unknown bookId", async () => {
    const res = await request(app)
      .get(`/books/general_information?bookId=10000`)
      .set(ID);
...
describe("DELETE books/share", () => {
  it("simple call", async () => {
    //Create book + share
    const res = await request(app)
      .post("/books/create?name=shareDelete")
      .set(ID)
      .send();
    expect(res.status).toBe(200);
    const check = await request(app)
      .get("/books/id?bookName=shareDelete")
      .set(ID)
      .send();
    expect(check.status).toBe(200);
    const share = await request(app)
      .post(`/books/share?bookId=${JSON.parse(check.text)["id"]}&userId=1`)
      .set(ID)
      .send();
    expect(share.status).toBe(200);
    //Try to delete share
    const deleteShareLink = await request(app)
      .delete(`/books/share?bookId=${JSON.parse(check.text)["id"]}&userId=1`)
      .set(ID);
    expect(deleteShareLink.status).toBe(200);
  });
  it("call on missing argument (bookId)", async () => {
    const deleteShareLink = await request(app)
      .delete(`/books/share?userId=1`)
      .set(ID);
    expect(deleteShareLink.status).toBe(400);
  });
  it("call on missing argument (userId)", async () => {
    const deleteShareLink = await request(app)
      .delete(`/books/share?bookId=1`)
      .set(ID);
    expect(deleteShareLink.status).toBe(400);
  });
});
...
describe("GET books/id", () => {
  it("call on unknown bookName", async () => {
    const res = await request(app)
      .get(`/books/id?bookName=unknownBookName`)
      .set(ID);
    expect(res.status).toBe(204);
  });
